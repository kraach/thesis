...
This was in order to conserve bolometer crate power.
 Due to the overheating of the gondola motor controller, the solar panels, which charged the bolometer crate batteries, were not facing the sun as often as was planned. 
...
In order for a chunk of data to pass the valid criteria, we required no more than 10\% of it be flagged. 
...
We predicted and measured the overbiased noise in units of $A/\sqrt{Hz}$.  
Because of the small value of the loopgain when the detector was overbiased, the responsivity was tiny and so the noise performance was dominated by the electronic, \ac{SQUID}, and Johnson noise. 
The electronic and \ac{SQUID} noise are calculated as before, using each detector's carrier and nuller settings. 
The predicted Johnson noise assumes the detector resistance is $R_{normal}$ from the network analysis fit and estimates the detector temperature as $T_{critical}$ + $\Delta P / \overline{G}$ from the dark critical transition temperature measurement and the IV curve at float, where $\Delta P$ was the difference in power from overbiased to in the transition. 
%Section~\ref{sec:parameter_measurements}. 
The prediction was calculated for each detector and Table~\ref{overbias_noise_table} reports the median overbiased noise prediction for each wafer in $pA/\sqrt{Hz}$. %\comred{How is overbias (both Johnson and readout noise) predicted? How do \ac{SQUID} and resistor noise inform our conclusion about the excess noise overbiased??}
