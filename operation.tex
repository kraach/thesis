...
\textcolor{red}{Need to go write the thermal conductance measurement section. (Be sure you explain how we get from IV to PV and how we extract $P_{electrical}$ and how wrong we might be about this number.)} 
...
\ee
and we measure $P_{total}$, the total power necessary to hold the detector in the negative electro-thermal feedback regime. 
...
The difference in the two powers measured, in the dark cryostat and open to light at float, gives each detector's measured radiative load,
\be
P_{radiative} = P_{electrical}^{dark} - P_{electrical}^{float},
\label{eq:radiative_load}
\ee
Figure~\ref{fig:dark_light_iv_curves} shows two PV curves for a 150~GHz detector dark in \ac{ETC} and open to light at float in \ac{EBEX}. 
...
The predicted radiative loads were generated by calculating the transmission, reflection, and absorption of each element in the detector's optical path \textcolor{red}{reference someone else's thesis? put in your own table?}. 
