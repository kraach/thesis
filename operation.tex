...
From the dark IV curve measurements described in Section~\ref{sec:thermal_conductance}, the power required to hold the detector in the constant electro-thermal feedback regime is determined.
\textcolor{red}{Need to go write the thermal conductance measurement section. (Be sure you explain how we get from IV to PV.)} 
Once \ac{EBEX} reached float altitude and the thick cryostat window was rolled back, IV curves were performed. 
...
The difference in the two powers measured, in the dark cryostat and open to light at float, gives each detector's measured radiative load, Figure~\ref{fig:dark_light_iv_curves}. 
