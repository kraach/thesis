...
Because of the small value of the loopgain when the detector was overbiased, the responsivity was tiny and so the noise performance was dominated by the electronic, \ac{SQUID}, and bolometer Johnson noise. 
The electronic and \ac{SQUID} noise were calculated as before, using each detector's carrier and nuller settings. 
The predicted Johnson noise assumed the detector resistance was $R_{normal}$ from the network analysis fit and estimated the detector temperature as $T_{bolo} = T_{c}$ + $\Delta P / \overline{G}$ from the dark critical transition temperature measurement and the IV curve at float, where $\Delta P$ was the difference in power from overbiased to in the transition. 
%Section~\ref{sec:parameter_measurements}. 
The prediction was calculated for each detector and Table~\ref{tab:overbias_noise_table} reports the median overbiased noise prediction for each wafer in $pA/\sqrt{Hz}$. %\comred{How is overbias (both Johnson and readout noise) predicted? How do \ac{SQUID} and resistor noise inform our conclusion about the excess noise overbiased??}
