...
We need to know the thermal conductance, $G$, of the thermal link from the TES to the bath in order to accurately predict sensitivity. 
The phonon \ac{NEP} is proportional to $G$, Equation~\ref{eq:nep}. 
We want to minimize $G$ such that we minimize our noise. 
This value, however, needs to be sufficiently high to allow for radiative load fluctuations.
When we set our target $G$, we included a safety factor of 2.5 times the expected load from the \ac{CMB}. 
...
\textcolor{red}{Discuss results. What are the major uncertainties in this measurement? Talk about how deep you drop and how you define depth in transition. (will address noise as function of depth in transition in dark noise section). Talk about dropping one bolometer at a time versus dropping the entire comb? i.e. talk about cross-talk? and how it's more dramatic when overbiased because the signal you're supplying is stronger? What happened when we measured $P_{elec}$ in ebex vs etc vs mcgill? How do we deal with different bath temperatures?}
