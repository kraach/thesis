...
To get $\overline G$ for each detector, we perform an IV curve where we step down the voltage bias, typically in steps of 0.05~$\mu V$, and measure the current through the bolometer. 
At each step, the instantaneous resistance, the ratio of the voltage bias to the current measured, is calculated. 
Each channel is dropped in voltage until the ratio of the voltage bias to the current measured reaches a fixed fraction of its original value.
In the test cryostats, the fraction was typically 0.7. 
For \ac{EBEX2013}, the fraction was 0.85 for the majority of flight because the noise performance observed deeper in the transition diverged from the predictions, \textcolor{red}{referenceGroundNoiseResults}.
