...
INCLUDE: STRAY INDUCTANCE FROM IV CURVE WITH LATCHED BOLOMETER
...
The detector sensitivity is a function of the thermal conductance, $G$, of the link from the TES to the bath, \textcolor{red}{FigureFromBoloTheoryThatDoesn'tExistYet}.
In particular, the phonon \ac{NEP} is proportional to $G$, Equation~\ref{eq:nep}. 
We want to minimize $G$ so that we minimize the detector \ac{NEP}. 
This value of $G$, however, needs to be sufficiently high to allow for the detectors to operate in the presence of the radiative load at float as well as any fluctuations in load, \textcolor{red}{referenceSectionAboutTargets}. 
The dark test cryostat measurements of $G$ allow us to predict the phonon \ac{NEP} for each detector and also determine if there will be sufficient dynamic range to operate the detectors under the expected loading conditions. 
...
In practice, we measure the average thermal conductance, $\overline G$, as opposed to the dynamic thermal conductance, $G$.
Then, knowing the bath temperature, the bolometer temperature, and the power of thermal conductivity, we use \textcolor{red}{EquationThatDoesn'tExistYet} to relate our measurement $\overline G$ to $G$. 
\textcolor{red}{some words about how get $\overline G$ from iv curve, i.e. it's not clear why you start talking about iv curves}
For each detector, we perform an IV curve where we step down the voltage bias, typically in steps of 0.05~$\mu V$, and measure the current through the bolometer. 
At each step, the instantaneous resistance, the ratio of the voltage bias to the current measured, is calculated. 
The bias voltage of each bolometer is dropped until the ratio of the voltage bias to the current measured reaches a fixed fraction of its original value.
In the test cryostats, the fraction was typically 0.7. 
For \ac{EBEX2013}, the fraction was 0.85 for the majority of flight.
The value used in flight was higher than the nominal lab value because in the test cryostats the bolometer noise performance deeper in the transition diverged from the predictions, \textcolor{red}{referenceGroundNoiseResults}.


\textcolor{red}{you need to explain how you get pv and rv from iv without making this sentence too terribly long. also, these words about saturation power are not clear.}

Example IV and PV curves from one detector on wafer 150-01 are shown in Figure~\ref{fig:bolo_iv_curve}.
Above the superconducting transition, at higher voltage biases, the detector is normal.
In this region, the AlTi bilayer of the \ac{TES} is normal and the current is proportional to $1 / R$. 
Fitting the normal region for this detector gives a resistance of 2.9 $\Omega$. 
As the bilayer begins to superconduct, the resistance drops and we see the current measured turn around with a characteristic smooth checkmark shape. 
Once the bolometer is sufficiently far into the superconducting transition, electro-thermal feedback causes the power dissipated in the bolometer to remain fixed, \textcolor{red}{referenceSectionOnBolometerTheory}.
...
Because each bolometer wafer is enclosed in a light-tight box for these tests, the total electrical power measured from the PV curve is often referred to as the saturation power, $P_{sat}$. 
Though $P_{sat}$ is not independent of  $G$, it can be a useful parameter to think in terms of because the sensitivity of the bolometer increases with lower saturation power (lower $G$), but the bolometer loses nearly all sensitivity if it saturates, i.e. its saturation power is less than the radiative load during observations. 
The average thermal conductance of the weak link is calculated from the saturation powers and critical temperatures, Section~\ref{sec:critical_temp}, 
\begin{equation}
\overline{G} = \frac{P_{sat}}{\Delta T}
\end{equation}
where $\Delta T$ is the difference between the critical and bath temperatures, $T_{c} - T_{bath}$. 
...
The bolometer is said to be at turnaround when the slope of the IV curve is zero. The power at turnaround is 2.1~pW. If the the slope of the PV curve becomes zero deep in the transition, then that value it levels off at is often called the saturation power. We approximate the saturation power as the last point in the detector's PV curve, 1.9~pW. 


\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\columnwidth]{figures/IV_201007301208.png} 
\caption{Left: The current through the \ac{SQUID} as function of the voltage bias across the bolometer. Right: The electrical power dissipated in the bolometer ($I / V$) as a function of the voltage bias across the bolometer. \textcolor{red}{I wish you could find or rewrite the code to make these figures - the pW are in square brackets and all the others in parenthesis. and the axis labels are smaller than I want them to be.}
\label{fig:bolo_iv_curve} }
\end{center}
\end{figure}


\textcolor{red}{Use this R vs V curve to help explain how deep in the transition the detectors are dropped. point out why it isn't the greatest way.}

\begin{figure}[htbp]
\begin{center}
\includegraphics[height=2in]{figures/RV_201007301208.png} 
\caption{The "resistance" of the bolometer ($I \over V$) as a function of the voltage bias across the bolometer.
