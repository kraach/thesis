...
In order to get the bolometer resonant frequencies and their normal resistances, we performed a network analysis on each comb with the bath held a couple hundred mK above $T_{c}$.
...
The impedance of each bolometer in series with its inductor, $L=24~\mu H$, and capacitor, $C_{i}$, was modeled as
\begin{equation}
Z_{RLC_{i}} = \sqrt{ R_{i}^{2} + \left(2\pi \nu L - \frac{1}{2\pi \nu C_{i}}\right)^{2}}
\end{equation}
where $R_{i}$ was the $i^{th}$ bolometer's normal resistance.
 and $C_{i}$ its capacitance. 
Each bolometer had a $24~\mu H$ inductor in series with it. 
There was also a stray inductance, $L_{x}$, and stray capacitance, $C_{x}$, in parallel with the detector comb. 
The network analysis current of each peak was thus
\begin{equation}
...
In addition to providing the resonant frequencies and normal resistances, the fit to this model gave the stray capacitance and inductance in parallel with the comb. 
Typical stray capacitance was about XXX $pF$ and typical stray inductance was XXX $uH$.
Note, the center of the current peak and the optimal frequency bias are not perfectly aligned because we wanted to maximize the current through the bolometer rather than maximize the current through the entire circuit (which included the stray capacitance and inductance).

\begin{figure}[htbp]
\begin{center}
\includegraphics[height=2.5in]{figures/SqCh3_netanal.png}
\caption{An 800~mK network analysis from wafer 150-17. The blue line has frequency steps of 10~kHz to roughly locate the peaks and the orange dots are the finer scans with steps of 1~kHz to obtain the data to fit. The fits provide the bolometer resonant frequencies, the bolometer normal resistances, and the circuit's stray impedance. 
