...
\textcolor{red}{Talk about the stray impedance effect you modeled.}
...
Each channel is dropped in voltage until the ratio of the voltage bias to the current measured reaches a fixed fraction of its original resistance, typically 85\% for the \ac{EBEX2013} flight.
In the case of a well-behaved bolometer with small stray impedance, electro-thermal feedback causes the power at the bolometer to remain fixed once the bolometer is sufficiently far into transition \cite{lanting_thesis} \textcolor{red}{fix citation}.
...
An example of the IV, PV, and RV curves is shown in Fig.~\ref{fig:bolo_iv_curve}, where the electrical power required to hold the detector in the transition is the horizontal section of the PV curve.
...
Though $P_{sat}$ is not independent of the $G$, it can be a useful parameter to think in terms of because the sensitivity of the bolometer increases with lower saturation power (lower $G$), but the bolometer loses nearly all sensitivity if it saturates, i.e. its saturation power is less than the radiative load during observations. 
