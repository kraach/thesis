...
We need to know the thermal conductance, $G$, of the thermal link from the TES to the bath in order to accurately predict the detector sensitivity. 
The phonon \ac{NEP} is proportional to $G$, Equation~\ref{eq:nep}. 
We want to minimize $G$ so that we minimize the contribution of the phonon noise to the total. 
This value, however, needs to be sufficiently high to allow for radiative load fluctuations.
When we set our target $G$, we included a safety factor of 2.5 times the expected load from the \ac{CMB} in case of excess load or fluctuations or other shit. 
...
In order to measure the thermal conductance, we perform IV curves. 
...
\textcolor{red}{Do you also want a plot of "resistance" versus voltage bias? It may be helpful if you want to explain how "deep" in the transition the detectors are dropped. i.e. to point out why it isn't the greatest way. but that can be a separate figure?}
