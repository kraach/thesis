...
\textcolor{red}{You need a circuit diagram. BUT. Do you need to repeat the circuit diagram with a stray L and C?? No. I don't think so. People can add those two components in their heads???}
...
\caption{An 800~mK network analysis from wafer 150-17. The blue line has frequency steps of 10~kHz to roughly locate the peaks and the orange dots are the finer scans with steps of 1~kHz to obtain the data to fit. 
...
\end{equation}
Figure~\ref{fig:netanal_fit} shows the fit of this model and a single peak.
...
\caption{The Lorentzian fit (blue line) to a single network analysis peak (red dots). The fit provides the bolometer resonant frequency (green dot), the bolometer normal resistance, and the circuit's stray inductance and stray capacitance. 
...
\textcolor{red}{Do you want to/ can you include any pictures or designs of the optics stack or teflon mould or box lid, etc ?}
...
In order to measure the optical efficiency of the \ac{EBEX} bolometers, we needed to provide a known, controllable radiative load.
We did this via a blackbody, a body absorbing all incident radiation falling upon it. 
The \ac{ETC} blackbody was an inverted cone with an angle of 37$^{\circ}$, Figure~\ref{fig:blackbody_design}, similar to the design used by Peterson and Richards in calibrating IR photometers \cite{Peterson1984a}. 
...
The light path for the ray with the outermost angle, the \ac{EBEX} 150~GHz detectors had a field of view of 20$^{\circ}$, still reflects off the blackbody surface four times.
%, $0.3  4 = XXX$ absorptivity. 
Fewer than 8 parts in 1000 of the incident radiation escapes the blackbody. 
...
\caption{Eccosorb blackbody design above detectors (black dots). The detector's field of view (red dashed lines) was 20$^{\circ}$. The blackbody cone had an opening angle of 37$^{\circ}$. The outermost light ray (blue line) bounces off the surface of the blackbody four times. 
