...
Though these detectors were far from those open to the blackbody, there could have been an optical load during that cryostat run.
The detector would have had to absorb 11~pW of radiation in order to bring the prediction in line with the measurement. 
...
Figure~\ref{fig:150-02_it_noise} shows a measurement performed as described above, except it was for a bolometer on wafer~150-02. 
The measurement at $0.9 \times R_{initial}$ is within 5\% of the prediction, which includes \ac{SQUID}, electronic, Johnson, and phonon noise (assuming large $\mathscr{L}$). 
...
\textcolor{red}{Re: high noise in ETC for 150-01. You have the data for photon noise if you have a 4K blackbody (1.7 pW) and you could show how the prediction changes if ALL of this power is absorbed by the (dark) detectors. Still doesn't explain excess.
what if it wasn't filtered? then it was 22 pw!!! that would certainly give more noise ...
}
