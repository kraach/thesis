...
The largest contributors were the first stage amplifier of the \ac{SQUID} controller board, $0.85~nV/\sqrt{Hz}$, and the $20~\Omega$ \ac{SQUID} controller board resistor Johnson noise, $0.57~nV/\sqrt{Hz}$. 
...
\textcolor{red}{do you want to mention what transimpedances we measured with the ebex squids? do you want to say HOW the value of $Z_{t} = dV/dI$ affects the noise predictions for those two components?}

With the \ac{SQUID}s on, the electronic noise predictions, specifically the noise due to the first stage amplifier and the 20~$\Omega$ resistor on the \ac{SQUID} controller board, required a measure of the \ac{SQUID} transimpedance, $dV/dI$. 
We measured $dV/dI$ from the slope of the linear regime of the $V-\phi$ curve where we bias the squid, Figure~\ref{fig:squid_transimpedance}. 
