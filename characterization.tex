...
\textcolor{red}{Is it worth the time to figure out why two of the electronic noise components are converted to current via $Z_{t}$ and the others use the squid flux-locked loop gain transfer function? I want to know! But is it worth it to answer that questions right now??}

With the \ac{SQUID}s on and their feedback loops closed, the electronic noise predictions were referred to current noise at the \ac{SQUID} input coil.
