...
\caption{The measured electronic and \ac{SQUID} noise (red and blue dots) as a function of demodulation frequency. The blue line is the noise prediction assuming the intrinsic \ac{SQUID} noise is 2.5~$pA/sqrt{Hz}$ while the orange line is assuming 3.5~$pA/sqrt{Hz}$. There 
\label{fig:adjust_squid} }
\end{center}
\end{figure}

Demodulating at the bolometer resonant frequencies gave a measure of the 4.2~K bolometers' Johnson noise in addition to the electronic and \ac{SQUID} noise. 
The 4.2~K Johnson noise roughly doubled the noise and did not agree with the predicted Johnson noise. 
At 700~mK, however, the Johnson noise did agree with the prediction, Figure~\ref{fig:}.
...
\begin{figure}[ht!]
\begin{center}
\includegraphics[height=2.5in]{figures/}
\caption{Electronic and \ac{SQUID} noise in \ac{ETC} as a function of demodulation frequency at 700~mK. Off resonance (dots) is just electronic and \ac{SQUID} noise, while on resonance (x's) includes the 700~mK bolometer Johnson noise. 
\label{fig:700mK_squid_noise} }
\end{center}
\end{figure}
