...
In practice, the radiative signal on the detector consists of a steady-state power $P_{0}$ and a time-varying signal of amplitude $\delta P$ and frequency $\omega$
\begin{equation}
...
\end{equation}
The total power in is the sum of the electrical power dissipated, $P_{elec}$ (including the change in electrical power due to the change in detector temperature), and the radiative power absorbed, $P_{rad}$,
\begin{equation}
...
\begin{equation}
P_{out}\left(T\right) = \overline{G}\Delta T + G \delta T e^{i\omega t} + i\omega C \delta T e^{i\omega t}
\label{eq:pout}
\end{equation}
...
To understand what happens to the power dissipated in the bolometer when its temperature changes by a small amount, we take the derivative of the power with respect to temperature, 
\begin{equation}
\begin{split}
