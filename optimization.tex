...
These types of detectors were used on ground-based telescopes, like XXX, XXX, and XXX. 
...
A superconductor is a special material which has a normal resistance until it reaches its critical temperature, at which point its resistance drops to zero, Figure~\ref{fig:r_vs_t}. 
...
The detector is voltage-biased, providing Joule heating to the detector. 
...
In response to the increase (decrease) in resistance, the Joule heating, $\frac{V^{2}}{R}$, decreases (increases) and the detector cools (warms). 
...
We then calibrate on a known source in the sky to determine how much change in current we measure for a given change in power.  
\textcolor{red}{There is some math to describe this more accurately.}
...
The phonon noise as a function of $T_{c}$, the power of thermal conductivity $n$,  the power to hold the detector in the transition $P_{sat}$, and the bath temperature $T_{bath}$ is
\begin{equation}
\label{eq:phonon_nep}
\begin{split}
...
\end{split}
\end{equation}
The phonon \ac{NEP} was minimized at a ratio of transition temperature to bath temperature of $\sim$1.7, Figure~\ref{fig:phonon_nep_vs_temps}. 
...
\caption{Phonon \ac{NEP} as a function of the ratio of critical temperature to bath temperature. The goal was to choose $T_{c}$ such that we minimize phonon noise.
...
The \ac{EBEX} detectors inherited their initial design from ground-based telescopes, the Atacama Pathfinder Experiment and the South Pole Telescope. 
...
The time constant needed to be held approximately constant because our telescope was to have roughly the same scan speed as the ground-based telescopes. 
Since the heat capacity, C, decreased by a factor of XXX (because there was no additional gold layered atop the bling), the thermal conductance, G, also need to be decreased by the same factor. 
