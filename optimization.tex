...
That is, the heat capacitive element absorbs radiation, warms up, and that change in temperature is measured with a thermistor.  
A \ac{TES} bolometer has a superconducting metal as its thermistor. 
A superconductor is a special metal which has a normal resistance until it reaches its critical temperature, at which point its resistance drops to zero, Figure~\ref{fig:r_vs_t}. 
...
\caption{Cartoon bolometer with incoming radiation, $\gamma$. There is an absorber with heat capacity, C, thermally coupled, with conductance G, to a reservoir at $T_{bath}$. A thermistor measures the change in temperature due to the power absorbed. \textcolor{red}{(Do you want to add a thermometer or something to the chunk of heat capacity to show that's what you're measuring the temperature of?)}
...
\caption{Resistance versus temperature for an \ac{EBEX} bolometer. As the temperature goes from high to low, the \ac{TES} goes from normal (finite resistance) to superconducting (zero resistance). The region in between is the transition and this is exactly where the \ac{TES} is biased for operation. 
