...
%IF YOU HAVE TIIME, COME BACK HERE AND EXPLAIN MORE
...
\caption{Left: One \ac{EBEX} 150~GHz spiderweb \ac{TES} bolometer. The light blue is the silicon wafer on which the detectors were fabricated. The dark blue is open space where the silicon was etched away. Running above the center of the etched out area are the thin silicon nitride strings of the spider web. The spider web is attached to the wafer at the end of each of the spokes. The gold waffle bling in the center provides the heat capacity. Right: A zoom of the bling and the \ac{TES}. The \ac{TES} is a square near the middle of the bling and is coupled to the bling via the overlap on its left side. The signal travels out the niobium leads to bond pads at the wafer's edge. Both figures courtesy of Benjamin Westbrook. 
...
The target normal resistance for all bands was 1.5~$\Omega$, Table~\ref{tab:Design_Params}.
The goal was to ensure the detector, when biased in the transition, remained in the stable regime where the detector electrical bandwidth, determined in part by the resistance, exceeded the \ac{TES} thermal bandwidth by at least XXX (CITE PAPER). 

The electrical cross-talk is also a function of the bolometer resistance. 
This needed to be subdominant to the optical cross-talk, due to beam side-lobes and radiation leakage within the detector integration cavity, which is typically at a level of $\sim$1\% \cite{Dobbs2011} . 
...
The electrical cross-talk is estimated by ~\cite{Dobbs2011}
\begin{equation}
\frac{R_{bolo}^2}{(2\Delta \omega L)^2}
\end{equation}
%see page 100 of notebook for calculation
For \ac{EBEX} we have inductors of $L_{EBEX} = 24~\mu H$ and frequency spacing of at least $\Delta \omega = 2\pi \cdot 60~kHz$. %$\Delta \nu = 60~kHz$. %
...
For a detector dropped to 85\% in the transition, a resistance of1.5~$\Omega$ results in cross-talk at a level of 0.5\%.
...
\end{equation}
We assumed $T_{bath}$ was 250~mK, $P_{sat}$ was 6~pW, and $n$ was in between one and three, Figure~\ref{fig:phonon_nep_vs_temps}. 
For $n=2$, the phonon \ac{NEP} was minimized at a ratio of transition temperature to bath temperature, $\frac{T_{c}}{T_{bath}}$, of $\sim$1.75. 
...
The \ac{EBEX} bath temperature was $\sim$250~mK and so the target critical temperature for all \ac{EBEX} observation frequency bands was 440~mK, Table~\ref{tab:Design_Params}.
...
Long-duration balloon flights, however, typically float at an altitude of $\sim$120,000~ft, above $\sim$98\% of the atmosphere. \textcolor{red}{reference CSBF, website? publication(s)?} 
...
Assuming bolometer absorption efficiencies of 0.6, 0.5, and 0.4 for the 150, 250, and 410~GHz bands, the predicted absorbed radiative powers were 1.4, 3.6, and 4.6 pW, respsectively.
...
In order to take advantage of the decrease in radiative load at float and increase the detector sensitivity, the fabrication goal was to lower the average thermal conductance for the 150~GHz detectors by a factor of $\sim$3 to 30~$pW/K$. 
...
The average thermal conductance goal for the 250~GHz was 40~$pW/K$ and for the 410~GHz detectors was 50~$pW/K$, Table~\ref{tab:Design_Params}.
