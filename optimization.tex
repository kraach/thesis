...
\textcolor{red}{Is this true? is it relevant?
...
N_{photon}^2 = 2 \int h \nu P_{\nu} d\nu + \xi \int \frac{P_{\nu}^{2}c^2}{A\Omega\nu^2} d\nu
\end{equation}
where $h$ is Planck's constant, $\nu$ is the frequency of the radiation, $P_{\nu}$ is the absorbed spectral power \cite{Richards}, and \xi is the so-called correlation factor. 
...
Note, this derivation assumed we have a diffraction limited beam so the throughput is $A\Omega = \lambda^2$, where $\Omega$ is the solid angle of a plane wave after passing through a circular aperture of area $A$ and $\lambda$ is the wavelength of the incident light. 
