...
\begin{figure}[ht!]
\begin{center}
\includegraphics[width=0.69\columnwidth]{images/board69_wire3_ch11_1356987555s_transition}
\includegraphics[width=0.29\columnwidth]{images/board69_wire3_ch11_1356987555s_aW}
...
\begin{figure}[ht!]
\begin{center}
\includegraphics[height=3in]{images/board69_wire3_ch11_noise_vs_time}
...
\begin{figure}[ht!]
\begin{center}
\includegraphics[height=3in]{images/150-09_NEP_median_meas_pred_distribution}
...
% do not include mapping speed for now 
%\subsubsubsection{Detector Mapping Speed}
%\label{sec:mapping_speed}
%
%
%The different parameters characterizing the bolometer wafers are used to compute the bolometer wafer mapping speed : a single value representing the sensitivity of the bolometer wafer to the expected absorbed signal.
%The mapping speed includes the effects of the spread of the bolometer resistances, critical temperatures and saturation powers on the sensitivity of a bolometer wafer and allows for the ranking of the bolometer wafers.
%Example histograms of the saturation powers and critical temperatures for the bolometer wafer 150-43 are shown in Fig.~\ref{fig:psat_histogram}.
%
%\begin{figure}
%\epsscale{0.8}
%\plotone{images/detectors_and_readout/Psat_Tc_histogram}
%\caption{{\it Left:} The distribution of saturation power for wafer 150-43. {\it Right:} The distribution of critical temperatures for wafer 150-43. \acomment{Will probably want to regenerate these plots as separate, non scaled images.}  \comgreen{Franky has code and data to regenerate these two plots.}}
%\label{fig:psat_histogram}
%\end{figure}
%
%The mapping speed of a single bolometer is
%\begin{equation}
%M_S^{bolo} = \frac{NEQ,U}{\sqrt{\Omega_s}},
%\end{equation}
%where NEQ,U is the noise equivalent temperature for the Q and U polarization and $\Omega_s$, the solid angle of the bolometer beam.
%%I'm here
%Combining all the mapping speeds of the bolometers on a bolometer wafer yields to the bolometer wafer mapping speed
%\begin{equation}
%M_S^{wafer} = \left( \sum_{i=1}^{n_b}{ \frac{1}{ \left( M_S^{bolo_i} \right)^2 } } \right)^{\frac{-1}{2}},
%\end{equation}
%where $n_b$ is the number of bolometers on the bolometer wafer.
%
%Approximately 45 bolometer wafers were characterized before \ac{EBEX2013}.
%A total of 14 were selected to form the two \ac{EBEX2013} focal planes.
%The end-to-end absorption efficiencies of the \ac{EBEX} experiement are calculated to be 21, 14 and 8.5\% for the 150, 250 and 410~GHz observing bands.
%Table~\ref{tab:mapping_speeds} shows the calculated mapping speed for the selected bolometer wafers.
%
%\begin{table}
%\begin{center}
%\begin{tabular}{| c | c | c |}
%\tableline
%Bolometer wafer & Bolometer NEQ,U & Mapping Speed \\
% & $\mu K \sqrt{s}$ & $\mu K \sqrt{{s \over deg^2}}$ \\ \tableline
%150-09 &  &  \\ \tableline
%150-14 &  &  \\ \tableline
%150-15 &  &  \\ \tableline
%150-20 &  &  \\ \tableline
...
%410-28 &  &  \\ \tableline
%\end{tabular}
%\caption{The mapping speeds of each bolometer wafer selected to compose the two \ac{EBEX2013} focal planes.  \comgreen{Franky waiting after first draft to compute this not to compute for nothing.}}
%\label{tab:mapping_speeds}
%\end{center}
%\end{table}
%
%% \newpage
%\comgreen{Comments/questions}
%\begin{itemize}
%  \item \comgreen{What do we use for $P_{rad}$ for the mapping speed?  Calculation pre-flight?  Measured value during flight? ...}
%  \item \comgreen{Do we go into any details for the end-to-end efficiency?  We should use a measured value instead of calculated value here if we chose to go with in-flight loading.}
%  \item \comgreen{Do we show mapping speed per band?  And show a noise ($\mu_{RMS}$) for the whole focal plane?}
%  \item \comgreen{Do we show a plot of the mapping speed VS the expected optical loading since the optical loading was uncertain before flight?}
%\end{itemize}
%


% make more unified so don't need these subsubsub sections 
%\subsubsubsection{Detector Noise Performance}
%\label{sec:noise_performance}
...
\begin{figure}
\plotone{images/2013-01-03--13-07-11_bolo_59-2-08chunk27block1_polyfit.png}
\caption{example TOD with polyfit}
\end{figure}

\begin{figure}
\plotone{images/2013-01-03--13-07-11_bolo_59-2-08chunk27block1_psfit.png}
\caption{example PSD with noise model fit}
\end{figure}

\begin{figure}
\plotone{images/perbolo_alltime_nets_69-1-12.png}
\caption{Example NET vs time for bolo 69-1-12, all flight}
\end{figure}

\begin{figure}
\plotone{images/perbolo_bolohist_nets_69-1-12.png}
\caption{Example NET distribution for bolo 69-1-12, all flight}
\end{figure}

\begin{figure}
\plotone{images/allbolomedian_nets_hist150.png}
\caption{Bolo (all flight) median NETs distribution (150 GHz)}
\end{figure}

\begin{figure}
\plotone{images/netssegmentnum150.png}
\caption{Bolo median NET per segment vs time (150 GHz)}
\end{figure}

\begin{figure}
\plotone{images/allbolomedian_fknees_hist150.png}
\caption{Bolo (all flight) median f knee distribution}
\end{figure}

\begin{figure}
\plotone{images/sens_mapsat150.png}
\caption{Sensitivity map at 150 GHz, nside = 64}
\end{figure}

\begin{figure}
\plotone{images/hist150cuthigh.png}
